...
RSpec.describe "Items", type: :request do
  ...
  # Ensure the seller is created for THIS user
  let(:my_seller) { user.seller }
  # Ensure the item is created for THIS seller
  ...
  describe "POST  /api/v1/items" do
        ...
        context "not authorized" do
              it "returns unauthorized" do
                post api_v1_items_path, params: valid_params # No headers passed
  ...
  describe "Patch /api/v1/item/id" do
      ...
      context "when user edits their own item" do
        it "updates the item and returns success" do
          ...
          expect(my_item.reload.title).to eq("Changed Title")
        end
      end
